...
func _test_extended_campaign_save_round_trip() -> void:
	...
	_assert_true(encoded.ok, "extended CampaignState encodes through the v1 save envelope")
	var decoded: Dictionary = codec.decode(encoded.text)
	_assert_true(decoded.ok, "extended CampaignState decodes through the v1 save envelope")
