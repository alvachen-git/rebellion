...
func _test_general_save_round_trip() -> void:
	...
	_assert_true(encoded.ok, "GeneralInstance CampaignState encodes through Save V1")
	var decoded: Dictionary = codec.decode(encoded.text)
	_assert_true(decoded.ok, "GeneralInstance CampaignState decodes through Save V1")
