...
func _build() -> void:
	...
	var troop_row := _status_bar("兵力", Tokens.CINNABAR)
	_troop_bar = troop_row.bar
	_troop_label = troop_row.label
	column.add_child(troop_row.root)
	var morale_row := _status_bar("士气", Tokens.MINERAL_GREEN)
	_morale_bar = morale_row.bar
	_morale_label = morale_row.label
	column.add_child(morale_row.root)
	...
	_detail_panel.position = Vector2(246, 38) if _side == Side.PLAYER else Vector2(-226, 38)
	_detail_panel.size = Vector2(218, 138)
	...
	_detail_label.add_theme_color_override("font_color", Tokens.INK)
	_detail_panel.add_child(_detail_label)


func _status_bar(label_text: String, fill_color: Color) -> Dictionary:
	var root := Control.new()
	root.custom_minimum_size = Vector2(0, 23)
	root.mouse_filter = Control.MOUSE_FILTER_IGNORE
	var bar := ProgressBar.new()
	bar.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	bar.mouse_filter = Control.MOUSE_FILTER_IGNORE
	bar.show_percentage = false
	bar.add_theme_stylebox_override("background", Tokens.panel_style(
		Tokens.with_alpha(Tokens.PAPER_BRIGHT, 0.9), Tokens.PAPER_SHADE, Tokens.RADIUS_SM, 1, 0
	))
	bar.add_theme_stylebox_override("fill", Tokens.panel_style(fill_color, fill_color, Tokens.RADIUS_SM, 0, 0))
	root.add_child(bar)
	var label := Label.new()
	...
	label.add_theme_color_override("font_color", Tokens.DEEP_TEAL_DARK)
	label.add_theme_constant_override("outline_size", 3)
	label.add_theme_color_override("font_outline_color", Tokens.with_alpha(Tokens.PAPER_BRIGHT, 0.94))
...
func _apply_snapshot() -> void:
	...
	_troop_label.text = "兵力  %d / %d" % [troops, max_troops]
	_morale_bar.max_value = max_morale
	_morale_bar.value = clampi(morale, 0, max_morale)
	_morale_label.text = "士气  %d / %d" % [morale, max_morale]
