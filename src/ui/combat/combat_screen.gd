...
@onready var setup_bar: Control = $Page/SetupBar
...
func _ready() -> void:
	...
	setup_bar.visible = not integrated_mode
	if integrated_mode:
		restart_button.text = "确认战果"
	if setup_battle(battle_request_override) and not integrated_mode:
		_populate_selection_controls()


func _apply_combat_style() -> void:
	$BackdropBase.color = Tokens.PAPER
	$Page/TopBar.theme_type_variation = "QingluPaperPanel"
	$Page/SetupBar.theme_type_variation = "QingluPaperPanel"
	$Page/Battlefield/IntentPanel.theme_type_variation = "QingluPaperPanel"
	$Page/TopBar/Row/Brand.theme_type_variation = "QingluHeading"
	turn_label.theme_type_variation = "QingluMuted"
	seed_label.theme_type_variation = "QingluCaption"
	...
	result_overlay.color = Tokens.with_alpha(Tokens.DEEP_TEAL_DARK, 0.92)
	result_detail.add_theme_color_override("font_color", Tokens.PAPER)
...
func restart_battle() -> void:
	if integrated_mode:
		if _controller != null and not _controller.is_active():
			battle_finished.emit(_controller.result())
		return
...
func _refresh() -> void:
	...
	enemy_combatant.configure(state.enemy, CombatantStageViewScript.Side.ENEMY)
	player_combatant.configure(state.player, CombatantStageViewScript.Side.PLAYER)
...
func _rebuild_hand(state: Dictionary) -> void:
	...
	for hand_index in state.deck.hand.size():
		...
		button.configure(card, {
			...
			"exhausted": card.get("tags", []).has("exhaust"),
		}, _controller.preview_card_damage(hand_index), CardViewScript.Density.FULL)
...
func _animate_enemy_resolution(intent: Dictionary, before: Dictionary, after: Dictionary) -> void:
	...
	if MotionPolicyScript.reduced():
		impact_label.position.y = impact_start_y
		return

	var movement := create_tween()
	var enemy_start_x: float = enemy_combatant.position.x
	if is_attack:
		movement.tween_property(enemy_combatant, "position:x", enemy_start_x - 30.0, 0.09).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
		movement.tween_property(enemy_combatant, "position:x", enemy_start_x, 0.16).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
		player_combatant.modulate = DANGER.lightened(0.22)
		var hit_flash := create_tween()
		hit_flash.tween_property(player_combatant, "modulate", Color.WHITE, 0.34)
	else:
		enemy_combatant.modulate = BRONZE.lightened(0.2)
		movement.tween_property(enemy_combatant, "modulate", Color.WHITE, 0.34)
